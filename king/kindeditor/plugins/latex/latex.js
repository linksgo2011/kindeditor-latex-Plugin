...
var LatexIT = {
    ...
    pre: function(txt) {
        if (!txt.match(/<img.*?>/i)) {
            ...
            if (this.mode == 'svg') {
                // Best for Firefox
                if (this.isFirefox) txt = " <object type=\"image/svg+xml\" data=\"http://latex.codecogs.com/" + this.mode + ".latex?" + txt + "\" class=\"latex\" style=\"margin:0; padding:0; border:0\" /> ";
            ...
            } else txt = " <img src=\"http://latex.codecogs.com/" + this.mode + ".latex?" + txt + "\" alt=\"" + txt + "\" border=\"0\" class=\"latex\" /> ";
        }
        return txt;
    },
    ...
    add: function(tag, latexmode) {
        ...
        if (window.addEventListener) window.addEventListener('load', new Function('LatexIT.render("' + tag + '", ' + latexmode + ')'), false);
        else window.attachEvent('onload', new Function('LatexIT.render("' + tag + '", ' + latexmode + ')'));
    },
...
KindEditor.plugin('latex', function(K) {
	var self = this, name = 'latex';
	self.clickToolbar(name, function() {
		var lang = self.lang(name + '.'),
			html = ['<div style="padding:10px 20px;">',
				...
				'请输入你的Latex表达式',
				'</div>',
				'<textarea class="ke-textarea" style="width:408px;height:100px;"></textarea>',
				'</div>'].join(''),
				...
				yesBtn : {
					...
					click : function(e) {
						var code = textarea.val();
 						var html = LatexIT.pre(code)
						if (K.trim(code) === '') {
							alert('请输入你的Latex表达式');
							textarea[0].focus();
							return;
						}
						self.insertHtml(html,true).hideDialog().focus();
					}
				},
				noBtn:false
			}),
